...
uniform highp sampler3D sparseGridIndex;
#endif
...
uniform float voxelSizeTriplane;
#endif
`;
 ...
 * @param fetchFn Function that fetches the file.
 * @return Returns fetchFn's response.
 */
 ...
 * @param filter Filter strategy of the texture
 ...
 * @param uploadFn Function that is called when a new volume slice is received.
 ...
 * @param filenameToLinkTranslator
...
function loadScene(
  ...
  filenameToLinkPromise.then(filenameToLink => {
    ...
    textPromises.then(parsed => {
      ...
      useSparseGrid = sceneParams['voxel_size'] > 0;
      let atlasIndexPromise;
      if (useSparseGrid) {
        // Load the indirection grid.
        let atlasIndexUrl =
            filenameToLinkTranslator.translate('atlas_indices.png');
        atlasIndexPromise = loadPNG(atlasIndexUrl);
        gNumTextures += 2 * sceneParams['num_slices'];
      }

      let planePromises = [];
      let useTriplane = 'voxel_size_triplane' in sceneParams;
      if (useTriplane) {
        gNumTextures += 6;
      ...
      let rgbVolumeTexture = null;
      let densityVolumeTexture = null;
      let featureVolumeTexture = null;
      let atlasIndexTexture = null;
      if (useSparseGrid) {
        ...
        // All of these textures have the same dimensions and use linear filtering
        rgbVolumeTexture = _createEmptyAtlasVolumeTexture(THREE.RGBFormat);
        densityVolumeTexture = _createEmptyAtlasVolumeTexture(THREE.RedFormat);
        featureVolumeTexture = _createEmptyAtlasVolumeTexture(THREE.RGBAFormat);

        // The indirection grid uses nearest filtering and is loaded in one go
        let w =
            Math.ceil(sceneParams['grid_width'] / sceneParams['block_size']);
        let h =
            Math.ceil(sceneParams['grid_height'] / sceneParams['block_size']);
        let d =
            Math.ceil(sceneParams['grid_depth'] / sceneParams['block_size']);
        atlasIndexTexture = createEmptyVolumeTexture(
            w, h, d, THREE.RGBFormat, THREE.NearestFilter);
        atlasIndexPromise.then(atlasIndexImage => {
          atlasIndexTexture.image.data = atlasIndexImage;
          atlasIndexTexture.needsUpdate = true;
        });
      }

      // Load triplanes
      let planeRgbTexture, planeDensityTexture, planeFeaturesTexture, planeSize;
      if (useTriplane) {
        let planeWidth = sceneParams['plane_width_0'];
        let planeHeight = sceneParams['plane_height_0'];
        planeSize = new THREE.Vector2(planeWidth, planeHeight);
        planeRgbTexture = createEmptyTriplaneTextureArray(
            planeWidth, planeHeight, THREE.RGBFormat);
        planeDensityTexture = createEmptyTriplaneTextureArray(
            planeWidth, planeHeight, THREE.RedFormat);
        planeFeaturesTexture = createEmptyTriplaneTextureArray(
            planeWidth, planeHeight, THREE.RGBAFormat);
        planePromises.then(planeData => {
          let planeRgbStack = new Uint8Array(3 * planeWidth * planeHeight * 3);
          let planeDensityStack = new Uint8Array(3 * planeWidth * planeHeight);
          let planeFeaturesStack = new Uint8Array(3 * planeWidth * planeHeight * 4);
          for (let plane_idx = 0; plane_idx < 3; plane_idx++) {
            let baseOffset = plane_idx * planeWidth * planeHeight;
            let planeRgbAndDensity = planeData[2 * plane_idx];
            let planeFeatures = planeData[2 * plane_idx + 1];
            for (let j = 0; j < planeWidth * planeHeight; j++) {
      ...
      let baseGridWidth, baseVoxelSize;
      if (useTriplane) {
        baseGridWidth = planeSize.x;
        baseVoxelSize = sceneParams['voxel_size_triplane'];
      } else {
        baseGridWidth = sceneParams['grid_width'];
        baseVoxelSize = sceneParams['voxel_size'];
      }
      let occupancyGridTextures = [];
      let occupancyGridSizes = [];
      let occupancyVoxelSizes = [];
      for (let occupancyGridIndex = 0; occupancyGridIndex < occupancyGridBlockSizes.length; occupancyGridIndex++) {
        ...
        // scene contraction
        let w = Math.ceil(baseGridWidth / occupancyGridBlockSize);
        let h = w;
        let d = w;
        let occupancyGridTexture = createEmptyVolumeTexture(w, h, d, THREE.RedFormat, THREE.NearestFilter);
        occupancyGridTextures.push(occupancyGridTexture);
        occupancyGridSizes.push(new THREE.Vector3(w, h, d));
        occupancyVoxelSizes.push(baseVoxelSize * occupancyGridBlockSize);
        occupancyGridPromises[occupancyGridIndex].then(
            occupancyGridImageFourChannels => {
              onImageLoaded();
              let occupancyGridImage = new Uint8Array(w * h * d);
              for (let j = 0; j < w * h * d; j++) {
      ...
      let M_dict = sceneParams['worldspace_T_opengl'];
      worldspaceROpengl['set'](
          M_dict[0][0], M_dict[0][1], M_dict[0][2], M_dict[1][0], M_dict[1][1],
          M_dict[1][2], M_dict[2][0], M_dict[2][1], M_dict[2][2]);

      let minPosition = new THREE.Vector3(
          sceneParams['min_x'], sceneParams['min_y'], sceneParams['min_z']);
      ...
      if (useTriplane) {
        let triplaneUniforms = {
          ...
          'voxelSizeTriplane': {'value': sceneParams['voxel_size_triplane']},
        };
      ...
      if (useSparseGrid) {
        let sparseGridUniforms = {
          'sparseGridDensity': {'value': densityVolumeTexture},
          'sparseGridRgb': {'value': rgbVolumeTexture},
          'sparseGridFeatures': {'value': featureVolumeTexture},
          'sparseGridIndex': {'value': atlasIndexTexture},
          'blockSize': {'value': sceneParams['block_size']},
          'voxelSize': {'value': sceneParams['voxel_size']},
          'gridSize': {
            'value': new THREE.Vector3(
                sceneParams['grid_width'], sceneParams['grid_height'],
                sceneParams['grid_depth'])
          },
          'atlasSize': {
            'value': new THREE.Vector3(
                sceneParams['atlas_width'], sceneParams['atlas_height'],
      ...
      requestAnimationFrame(
          t => update(t, dirUrl, filenameToLinkTranslator, useSparseGrid, sceneParams));
    });
  });
}
...
function loadOnFirstFrame(
  ...
  if (useSparseGrid) {
    ...
    const densityVolumeTexture = _getUniform('sparseGridDensity');
    const rgbVolumeTexture = _getUniform('sparseGridRgb');
    const featureVolumeTexture = _getUniform('sparseGridFeatures');
    ...
    let uploadFn = (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
      ...
      uploadVolumeSlice(rgbPixels, rgbVolumeTexture, sliceIndex, volumeWidth, volumeHeight, sliceDepth);
      uploadVolumeSlice(densityPixels, densityVolumeTexture, sliceIndex, volumeWidth, volumeHeight, sliceDepth);
    };
    let rgbAndDensityVolumeTexturePromise = _loadVolumeTextureSliceBySlice(uploadFn, 'rgba');

    // The pngs RGBA channels are directly interpreted as four feature channels
    uploadFn = (rgbaImage, sliceIndex, volumeWidth, volumeHeight, sliceDepth) => {
      uploadVolumeSlice(rgbaImage, featureVolumeTexture, sliceIndex, volumeWidth, volumeHeight, sliceDepth);
    };
    featureVolumeTexturePromise = _loadVolumeTextureSliceBySlice(uploadFn, 'feature');

    let allTexturesPromise = Promise.all([rgbAndDensityVolumeTexturePromise, featureVolumeTexturePromise]);
