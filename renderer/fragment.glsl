...
void main() {
  ...
  while (step < maxStep && visibility > 1.0 / 255.0) {
...
#ifdef USE_SPARSE_GRID
    // handle the atlas block
    vec3 atlasBlockMin = floor(posSparseGrid / blockSize) * blockSize;
    vec3 atlasBlockMax = atlasBlockMin + blockSize;
    // (atlasBlockMin + atlasBlockMax) / 2.0 / blockGridSize, first part is mean of atlas coord, then normalized to [0, 1]
    // this is the median index of the occupied block
    vec3 atlasBlockIndex = 255.0 * texture(sparseGridIndex, (atlasBlockMin + atlasBlockMax) / (2.0 * blockGridSize)).xyz;
    ...
    // +1, +0.5 because it's not border aligned
