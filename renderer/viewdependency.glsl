float indexToPosEnc(vec3 dir, int index) {
  float coordinate = (index % 3 == 0) ? dir.x : ((index % 3 == 1) ? dir.y : dir.z);
  if (index < 3) return coordinate;
  // modified to [sin(x), cos(x), sin(2x), cos(2x), ...] instead of [sin(x), sin(2x), ... cos(x), cos(2x), ...]
  int scaleExponent = (index - 3) / 6;
